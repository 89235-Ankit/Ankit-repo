...
const Wishlist = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/api/favorites/${userId}`)
      .then((res) => {
        setFavoriteCarIds(res.data); // res.data should be an array like [1,2,3]
      })
  ...
  useEffect(() => {
    const fetchAllCarsById = async (carIds) => {
      try {
        const carDetailsPromises = carIds.map(id =>
          fetch(`http://localhost:8080/api/cars/${id}`).then(res => res.json())
        );
        const carDetailsList = await Promise.all(carDetailsPromises);
        setCars(carDetailsList);
      } catch (err) {
    ...
    if (favoriteCarIds.length > 0) {
      fetchAllCarsById(favoriteCarIds);
    }
  }, []);
