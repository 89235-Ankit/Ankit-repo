...
const CarDetailPage = () => {
  ...
  const toggleWishlist = () => {
    ...
    axios.post(`http://localhost:8080/api/favorites/3/${carId}`)
      .then(res => {
        if (res.status === 200) {
      ...
      <div className="row g-4">
        ...
        <div className="col-md-5">
          <div className="image-wrapper position-relative">
            ...
            {car.images.length > 1 && (
              <>
                <button className="image-nav-button left btn btn-light rounded-circle" onClick={prevImage}>
                  <i className="bi bi-chevron-left fs-4" />
                </button>
                <button className="image-nav-button right btn btn-light rounded-circle" onClick={nextImage}>
          ...
          <div className="d-flex justify-content-between align-items-start mt-4">
            ...
            <button className="btn btn-link p-0" onClick={toggleWishlist}>
              <i className={`bi ${wishlist ? 'bi-heart-fill text-danger' : 'bi-heart text-muted'} fs-3`} />
            </button>
          </div>
          <p>{car.description}</p>
        ...
        <div className="col-md-7">
          <h4>{car.brand} {car.model} - {car.variant}</h4>
          <table className="table table-bordered table-sm mt-3 fs-5">
            <tbody>
              <tr><th>Registration Year</th><td>{car.registrationYear}</td></tr>
              <tr><th>Ownership</th><td>{car.ownership}</td></tr>
              <tr><th>KM Driven</th><td>{car.kmDriven} km</td></tr>
              <tr><th>Location</th><td>{car.location}</td></tr>
              <tr><th>Registration No.</th><td>{car.registrationNumber}</td></tr>
              <tr><th>Color</th><td>{car.color}</td></tr>
              <tr><th>Insurance Valid</th><td>{car.insuranceValid ? 'Yes' : 'No'}</td></tr>
              ...
              <tr><th>Listed On</th><td>{new Date(car.createdAt).toLocaleDateString()}</td></tr>
            </tbody>
          </table>
        </div>
      </div>
